...
function MinCaloroies() {
  ...
  const gethighCals = async () => {
    try {
      const response = await fetch(
        `https://api.spoonacular.com/recipes/findByNutrients?&minProtein=30&minCalories=600&number=20&apiKey=${process.env.REACT_APP_SPOONACULAR_API_KEY}`
      );

      const data = await response.json();
      sethighCalories(data);

      console.log(data);
      console.log(`these are the recipies`, highcalories);
      if (!response.ok) {
  ...
  return (
    <Wrapper className="highProtein">
      <h2 className="h1protein"> High Calorie Recipies</h2>
      <Splide
        className="splide"
        options={{
          perPage: 3,
